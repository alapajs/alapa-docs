...
<CodeScreen title="src/app/api/books/Controller.ts">

```ts
import { RestfulResource, Request, Response } from "alapa";
import { Book } from "./Models"; // Assuming Book is a model representing your database entity.

export class BookController implements RestfulResource {
  // List all books
  async index(req: Request, res: Response) {
    const books = await Book.findAll();
    res.json(books);
...
import { BookController } from "./ApiController";
...
bookRoutes.resource("books", BookController, {
  changeNamesWithVerbs: false, // User the default route names
});
```
...
```ts
bookRoutes.resource("books", BookController, {
  createNames: false, // Exclude create route names
});
```
